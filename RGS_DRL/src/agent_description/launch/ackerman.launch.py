...
def generate_launch_description():
    ...
    ackerman_path = PathJoinSubstitution([agent_decription_path, "urdf", "ackerman.xacro"])
    world_path = PathJoinSubstitution([agent_decription_path, "worlds", "empty.world"])

    # 使用 xacro 命令读取 xacro/urdf
    ackerman_model = Command(["xacro ", ackerman_path])
    ...
    robot_state_publisher_node = Node(
        ...
        output="screen",
        parameters=[{"robot_description": ackerman_model}]
    )
